...
public class BTreeImpl <Key extends Comparable<Key>,Value> implements BTree<Key,Value> {
    ...
    public Value get(Key k)  {
        ...
        if(entry != null)
        {
            ...
            //value of the key is null, have to check if its on the disk
            //if value is found on disk, deserialize and return it
            // if value is NOT on disk, value never existed or was delted and will return null
...
//                    throw new RuntimeException(e);
                ...
                //returning the deserialized value
                return (Value) entry.val;
            }

            //value was in btree, return it
            return (Value)entry.val;
        }
        //not in btree or disk
        return null;
    }
    ...
    public Value put(Key k, Value v) {
        ...
        //delete by put(Key,null)
        //if the key already exists in the b-tree, simply replace the value and return old
        Entry alreadyThere = this.get(this.root, k, this.height);
        if(alreadyThere != null) {
            Value old = (Value) alreadyThere.val;
            alreadyThere.val = v;

            //delete doc if its was on disk
            try {
        ...
        // key way new and no old value exists
        return null;
    }
    ...
    public void moveToDisk(Key k) throws IOException {
        //have to move doc to disk

        //get the document
        Value v = this.get(k);

        //null out the document Value so you know its on disk
        this.put(k,null);
